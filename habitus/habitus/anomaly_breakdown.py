...
def score_entities(current_states: dict = None) -> list:
    ...
    for eid, bl in baselines.items():
        ...
        b = bl[key]
        if b['std'] < 0.01: continue  # constant sensor, skip
        ...
        if z < 0.5: continue  # not interesting
        ...
        if r.status_code == 200:
            for s in r.json():
                if s['entity_id'] in entity_ids:
                    try: result[s['entity_id']] = float(s['state'])
...
def _guess_unit(eid):
    e = eid.lower()
    if 'temperature' in e: return '°C'
    if 'humidity' in e: return '%'
    if '_w' in e or 'watt' in e or 'power' in e: return 'W'
    if 'energy' in e or 'kwh' in e: return 'kWh'
    if 'current' in e or '_a' in e: return 'A'
    if 'voltage' in e: return 'V'
    if 'pressure' in e: return 'hPa'
    return ''
