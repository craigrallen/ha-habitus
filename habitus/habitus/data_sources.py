...
def fetch_hybrid(
    ...
    for entity_id in entity_ids:
        ...
        try:
            ...
            states_hourly = {}
            for _eid, ts, state_val in states_rows:
                try:
                    val = float(state_val)
    ...
    for entity_id in entity_ids:
        ent_stats = (
            stats_df[stats_df["entity_id"] == entity_id] if not stats_df.empty else pd.DataFrame()
        )
        ent_states = (
            states_df[states_df["entity_id"] == entity_id]
            if not states_df.empty
            else pd.DataFrame()
        )
