...
"""
import asyncio, json, os, datetime, argparse, pickle, logging
import pandas as pd
import numpy as np
import requests
...
def set_progress(phase, done=0, total=0, rows=0, elapsed=0.0, eta=0.0):
    try:
        ...
        with open(PROGRESS_PATH,'w') as f:
            json.dump({"running":True,"phase":phase,"done":done,"total":total,
                       "pct":pct,"rows":rows,"elapsed_min":round(elapsed/60,1),
                       "eta_min":round(eta/60,1)}, f)
    ...
    for eid in entity_ids:
        try:
            ...
            await ws.send(json.dumps({"id":1,"type":"recorder/statistics_during_period",
                ...
                "statistic_ids":[eid],"period":"hour","types":["mean","sum"]}))
            ...
            for sid, points in result.get('result',{}).items():
                for p in points:
                    ts = p['start']
                    if ts > 1e10: ts /= 1000
                    all_rows.append({'entity_id':sid,'ts':ts,'mean':p.get('mean'),'sum':p.get('sum')})
            done += 1
...
def build_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    power = df[df['entity_id'].str.contains('consumed_w|watt|power|inverter|load', case=False, na=False)].copy()
...
def send_notification(title, message):
    if not NOTIFY_ON: return
    headers = {"Authorization":f"Bearer {HA_TOKEN}","Content-Type":"application/json"}
    service = NOTIFY_SVC.replace('.','/')
    try:
        r = requests.post(f"{HA_URL}/api/services/{service}",
                          headers=headers, json={"title":title,"message":message}, timeout=5)
...
def publish(entity_id, state, attributes=None):
    ...
    try:
        r = requests.post(f"{HA_URL}/api/states/{entity_id}",
                          headers=headers, json={"state":str(state),"attributes":attributes or {}}, timeout=5)
        if r.status_code not in (200,201): log.error(f"Publish {entity_id}: {r.status_code}")
...
async def run(days_history: int, mode: str = "full") -> None:
    ...
    if state.get('data_to') and os.path.exists(MODEL_PATH):
        ...
        if df_new.empty or len(df_new) < 24:
            ...
            features_tmp = build_features(df_new) if not df_new.empty else pd.DataFrame()
            anomaly_score = seasonal.score_with_best_model(
                np.array([[datetime.datetime.now().hour, datetime.datetime.now().weekday(),
                           int(datetime.datetime.now().weekday()>=5),
                           datetime.datetime.now().month, 0, 0, 0]]))[0]
            training_days = state.get('training_days', 0)
            ...
            if df_full.empty: log.warning("No data"); return
    ...
    state.update({'last_run':now_iso,'data_to':now_iso,'training_days':training_days,
                  'entity_count':entity_count,'anomaly_score':anomaly_score,
